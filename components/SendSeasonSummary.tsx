...
export function SendSeasonSummary({
  ...
  return (
    <div className="rounded-2xl border border-line bg-surface p-4">
      <p className="mb-2 font-extrabold text-ink">Compartir en Discord</p>
      <p className="mb-3 text-xs text-ink-soft">
        {disabled
          ? "Disponible cuando la liga tenga rondas jugadas."
          : "Publica el cuadro de estrellas por ronda de toda la temporada."}
      </p>
      <div className="flex items-center gap-2">
        {channels.length > 0 && (
          <select
            ...
            disabled={disabled}
            className="min-w-0 flex-1 rounded-lg border border-line bg-surface-2 px-2.5 py-2 text-sm font-semibold text-ink outline-none focus:border-gold disabled:opacity-50"
          >
            {channels.map((c) => (
